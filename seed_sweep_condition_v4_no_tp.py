...
def _simulate_seed(seed: int) -> dict[str, Any]:
    ...
    for idx, d in enumerate(dates):
        ...
        for p in positions:
            ...
            _, _, current_close = px
            p.holding_days += 1

            reason: str | None = None
            if stop_loss_pct > 0 and current_close <= p.entry_price * (1.0 - stop_loss_pct):
                reason = "percent_stop"
            ...
            gross_sale = p.quantity * current_close
...
def main() -> None:
    ...
    parser.description = "MA240 < MA120 < MA60 sweep with no take-profit, optional percent stop, and Kiwoom costs."
    parser.add_argument(
        "--stop-loss-pct",
        type=float,
        default=0.07,
        help="Close-based stop fraction. Use 0 to disable stop-loss.",
    )
    ...
    parser.set_defaults(
        ...
        sell_tax_rate=0.0,
        max_hold_days=20,
        output_dir="results/seed_sweep_condition_v4_no_tp",
    )
    ...
    if not 0 <= args.stop_loss_pct < 1:
        raise ValueError("stop-loss-pct must be between 0 and 1; use 0 to disable")
    ...
    stop_text = "disabled" if args.stop_loss_pct == 0 else f"-{args.stop_loss_pct * 100:.2f}% close"
    print(f"strategy: no take-profit / stop {stop_text} / max hold {config.max_hold_days} days")
    ...
    aggregate["exit_model"] = {
        "take_profit": None,
        "stop_loss_pct": args.stop_loss_pct if args.stop_loss_pct > 0 else None,
        "stop_trigger": "daily_close" if args.stop_loss_pct > 0 else None,
    ...
    aggregate["exit_stats"] = {
        "mean_percent_stop_count": round(float(results["percent_stop_count"].mean()), 2),
        "mean_max_hold_exit_count": round(float(results["max_hold_exit_count"].mean()), 2),
    }
    ...
    print("\n=== Condition v4 no-TP + Kiwoom cost summary ===")
