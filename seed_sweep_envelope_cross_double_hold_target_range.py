...
import seed_sweep_envelope_cross_half_hold as cross_base
...
def build_parser():
    ...
    parser.description = (
        "Diagnostic sweep using a recent LOW-based Envelope lower-band downward cross, "
        "double recent-touch holding period, and an entry-time planned target-return range."
    )
    parser.add_argument(
        "--envelope-cross-lookback-days",
        type=int,
        default=3,
        help="Require a LOW-based lower-band downward cross within the most recent N trading sessions. Default: 3.",
    )
    parser.add_argument(
        "--hold-period-ratio",
        type=float,
        default=2.0,
        help="Multiply the original recent-target-touch holding period by this ratio. Default: 2.0.",
    )
    parser.add_argument(
        "--planned-target-return-min",
        type=float,
        default=8.0,
        help="Minimum target return from the actual next-day entry price, in percent. Default: 8.",
    )
    parser.add_argument(
        "--planned-target-return-max",
        type=float,
        default=16.0,
        help="Maximum target return from the actual next-day entry price, in percent. Default: 16.",
    )
    parser.set_defaults(output_dir="results/seed_sweep_envelope_cross_double_hold_target_range")
    return parser
...
def validate_args(args) -> None:
    ...
    if args.envelope_period is None:
        raise ValueError("This experiment requires --envelope-period and --envelope-percent.")
    if args.envelope_cross_lookback_days < 1:
        raise ValueError("envelope-cross-lookback-days must be >= 1")
    if args.hold_period_ratio <= 0:
        raise ValueError("hold-period-ratio must be > 0")
    if args.planned_target_return_min < 0:
        raise ValueError("planned-target-return-min must be >= 0")
    if args.planned_target_return_max <= args.planned_target_return_min:
        raise ValueError("planned-target-return-max must be greater than planned-target-return-min")


def filter_dict(args) -> dict[str, Any]:
    out = unified._filter_dict(args)
    out.update(
        {
            "envelope_cross_basis": "low",
            "envelope_cross_lookback_days": args.envelope_cross_lookback_days,
            "hold_period_ratio": args.hold_period_ratio,
            "planned_target_return_min": args.planned_target_return_min,
            "planned_target_return_max": args.planned_target_return_max,
        }
    )
    return out
...
def prepare(args):
    # Reuse the LOW-based recent-cross preparation. It already calculates the
    # recent-target-touch age and scales it by hold_period_ratio. A ratio > 1 is
    # valid here even though the older half-hold runner's CLI validator capped it at 1.
    return cross_base.prepare(args)
...
def _simulate_seed(seed: int) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for idx, d in enumerate(dates):
        ...
        for ticker, signal_date, meta in entering:
            ...
            # Apply the requested 8%-16% filter using the ACTUAL next-day entry
            # price, not the signal-day close, so it matches trade diagnostics.
            if target is not None:
                planned_target_return = (target / entry - 1.0) * 100.0
                if not (
                    a["planned_target_return_min"]
                    <= planned_target_return
                    <= a["planned_target_return_max"]
                ):
        ...
        survivors: list[diagnostics.DiagnosticPosition] = []
        for p in positions:
...
def main() -> None:
    ...
    print("[1/2] preparing recent LOW-based Envelope-cross candidate set...")
    prepared, prepared_path, reused = prepare(args)
    print(f"prepared cache: {prepared_path} ({'reused' if reused else 'built'})")
    print("filters:")
    print(json.dumps(filter_dict(args), ensure_ascii=False, indent=2))
    print(
        f"entry target-return filter: {args.planned_target_return_min:g}% "
        f"to {args.planned_target_return_max:g}% from actual next-day entry"
    )
    print(f"hold: recent-touch age * {args.hold_period_ratio:g}")
    ...
    summary["filters"] = filter_dict(args)
    summary["planned_target_return_filter_pct"] = {
        "min": args.planned_target_return_min,
        "max": args.planned_target_return_max,
        "basis": "actual_next_day_entry",
    }
    summary["workers"] = workers
    summary["prepared_cache"] = str(prepared_path)
    summary["mean_skipped_target_return_range"] = round(float(results["skipped_target_return_range"].mean()), 2)
    summary["trade_diagnostics"] = diagnostics.build_trade_diagnostics(trades_df)
    with (out / "seed_summary.json").open("w", encoding="utf-8") as fh:
        json.dump(summary, fh, ensure_ascii=False, indent=2)

    print("\n=== Double-hold target-range sweep summary ===")
