...
def _cache_path(config: Config, ma_fast: int, ma_mid: int, ma_trend: int, ma_long: int) -> Path:
    key = {
        "start": config.start,
        "end": config.end,
        ...
        "ma_trend": ma_trend,
        "ma_long": ma_long,
        "condition": "ma_long<ma_trend and ma_trend>ma_fast<ma_mid",
        "format_version": 1,
    }
    digest = hashlib.sha256(
        json.dumps(key, sort_keys=True, default=list).encode("utf-8")
    ).hexdigest()[:16]
    root = Path(config.cache_dir) / "prepared_condition_v2"
    root.mkdir(parents=True, exist_ok=True)
    return root / f"{digest}.pkl"


def prepare_updated_condition(
    config: Config,
    ma_fast: int,
    ma_mid: int,
    ma_trend: int,
    ...
    cache_path = _cache_path(config, ma_fast, ma_mid, ma_trend, ma_long)
    ...
    # 120 trading days is roughly six months; use ample calendar-day warmup.
    warmup_start = start - pd.DateOffset(days=max(400, ma_long * 3))
    ...
    for days in sorted({ma_fast, ma_mid, ma_trend, ma_long}):
    ...
    eligible = (
        ...
        # Screenshot: 120 MA < 60 MA
        & (signal_frame[f"ma_{ma_long}"] < signal_frame[f"ma_{ma_trend}"])
        # Screenshot: 60 MA > 3 MA < 10 MA
        & (signal_frame[f"ma_{ma_trend}"] > signal_frame[f"ma_{ma_fast}"])
        & (signal_frame[f"ma_{ma_fast}"] < signal_frame[f"ma_{ma_mid}"])
    )
...
def main() -> None:
    parser = build_parser()
    parser.description = "Multi-seed sweep for the updated MTS condition with percentage stop-loss."
    parser.add_argument("--stop-loss-pct", type=float, default=0.05)
    parser.add_argument("--ma-fast", type=int, default=3)
    parser.add_argument("--ma-mid", type=int, default=10)
    parser.add_argument("--ma-trend", type=int, default=60)
    parser.add_argument("--ma-long", type=int, default=120)
    parser.set_defaults(output_dir="results/seed_sweep_condition_v2")
    ...
    if min(args.ma_fast, args.ma_mid, args.ma_trend, args.ma_long) < 1:
        raise ValueError("moving-average periods must be >= 1")
    ...
    print("[1/2] preparing updated-condition candidates once...")
    prepared, prepared_path, reused = prepare_updated_condition(
        config,
        args.ma_fast,
        args.ma_mid,
        args.ma_trend,
    ...
    print(
        f"condition: MA{args.ma_long} < MA{args.ma_trend}, "
        f"MA{args.ma_trend} > MA{args.ma_fast} < MA{args.ma_mid}"
    )
    ...
    aggregate = _summarize(
        results, config, args.seed_start, args.seed_end, args.stop_loss_pct
    )
    aggregate["condition"] = {
        "ma_long_lt_ma_trend": [args.ma_long, args.ma_trend],
        "ma_trend_gt_ma_fast_lt_ma_mid": [
            args.ma_trend,
            args.ma_fast,
            args.ma_mid,
        ],
    ...
    print("\n=== Updated-condition percent-stop summary ===")
