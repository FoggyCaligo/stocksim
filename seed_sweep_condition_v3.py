...
def _cache_path(
    ...
    key = {
        ...
        "ma_trend": ma_trend,
        "ma_long": ma_long,
        "condition": "ma120<ma60<ma3<ma10 and ma5<ma10",
        "format_version": 1,
    }
    digest = hashlib.sha256(
        json.dumps(key, sort_keys=True, default=list).encode("utf-8")
    ).hexdigest()[:16]
    root = Path(config.cache_dir) / "prepared_condition_v3"
    root.mkdir(parents=True, exist_ok=True)
    return root / f"{digest}.pkl"


def prepare_condition_v3(
    config: Config,
    ma_fast: int,
    ma_short: int,
    ma_mid: int,
    ...
    cache_path = _cache_path(
        config, ma_fast, ma_short, ma_mid, ma_trend, ma_long
    )
    ...
    for days in sorted({ma_fast, ma_short, ma_mid, ma_trend, ma_long}):
...
def main() -> None:
    parser = build_parser()
    parser.description = "Multi-seed sweep for the revised MTS condition with percentage stop-loss."
    ...
    parser.set_defaults(output_dir="results/seed_sweep_condition_v3")
    ...
    if min(args.ma_fast, args.ma_short, args.ma_mid, args.ma_trend, args.ma_long) < 1:
    ...
    print("[1/2] preparing revised-condition candidates once...")
    prepared, prepared_path, reused = prepare_condition_v3(
        config,
        args.ma_fast,
        args.ma_short,
        args.ma_mid,
    ...
    print(
        f"condition: MA{args.ma_long} < MA{args.ma_trend} < MA{args.ma_fast} < MA{args.ma_mid}, "
        f"MA{args.ma_short} < MA{args.ma_mid}"
    )
    ...
    aggregate["condition"] = {
        ...
        "ma_trend_lt_ma_fast_lt_ma_mid": [
            args.ma_trend,
            args.ma_fast,
            args.ma_mid,
        ],
        "ma_short_lt_ma_mid": [args.ma_short, args.ma_mid],
    }
    ...
    print("\n=== Revised-condition percent-stop summary ===")
